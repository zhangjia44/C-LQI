...
int general_work(int noutput, gr_vector_int& ninput_items,
	...
	while(!d_symbols_offset) {
		...
		if(pmt::is_pair(msg)) {
			...
			std::cout << "length=" << psdu_length << std::endl;
			// ############ INSERT MAC STUFF
			...
			/*frame.print();
			FILE *psdu_result = fopen("psdu_result", "wb");
			for(int j = 0; j < 5452; j++)
				fputc(psdu[j], psdu_result);
			FILE *bit_result = fopen("bit_result", "wb");
			for(int j = 0; j < frame.n_data_bits; j++)
				fputc(data_bits[j], bit_result);
			FILE *scramble_result = fopen("scramble_result", "wb");
			for(int j = 0; j < frame.n_data_bits; j++)
				fputc(scrambled_data[j], scramble_result);
			FILE *convolutional_result = fopen("convolutional_result", "wb");
			for(int j = 0; j < frame.n_encoded_bits; j++)
				fputc(punctured_data[j], convolutional_result);
			FILE *interleave_result = fopen("interleave_result", "wb");
			for(int j = 0; j < frame.n_encoded_bits; j++)
				fputc(interleaved_data[j], interleave_result);
			FILE *symbols_result = fopen("symbols_result", "wb");
			for(int j = 0; j < frame.n_encoded_bits / d_ofdm.n_bpsc; j++)
				fputc(symbols[j], symbols_result);
			fclose(scramble_result);
			fclose(bit_result);
			fclose(convolutional_result);
			fclose(interleave_result);
			fclose(symbols_result);*/
