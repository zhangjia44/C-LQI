...
unsigned char decode_chips(unsigned int chips){
	...
	std::ofstream ofile;
	ofile.open("symbol_with_chips.txt", std::ios::app);
	ofile << "errors number: ";
	for(i=0; i<16; i++) {
		...
		if(threshold < 10)
			ofile << char('0' + threshold) << " ";
		else
		{
			ofile << char('0' + threshold / 10) << char('0' + threshold % 10) << " ";
		}
	...
	if (min_threshold < d_threshold) {
		...
		int out_char = best_match & 0xF;
		ofile << "   phase shift: ";
		for(int it = 0; it < 32; it++) {
			ofile << !! int(chips & (1 << (31 - it)));
		}
		ofile << "   symbol: ";
		if(out_char < 10) {
			ofile << (char)('0' + out_char) << " ";
		}
		else {
			ofile << (char)('A' + out_char % 10) << " ";
		}
		ofile << "   cnt: " << min_threshold << std::endl;
		return (char)best_match & 0xF;
	}
	ofile << "wrong byte " << std::endl;
	return 0xFF;
}
	...
	while(count < ninput) {
		...
		case STATE_SYNC_SEARCH:    // Look for sync vector
			...
			while (count < ninput) {
				...
				if(d_preamble_cnt == 0){
					unsigned char my_s = decode_chips(d_shift_reg);
					if(my_s == 0x00) {
		...
		case STATE_HAVE_HEADER:
			...
			while (count < ninput) {   // shift bits into bytes of packet one at a time
				...
				if(d_chip_cnt == 31){
					...
					fprintf(stderr, "%02X ||  ", c), fflush(stderr);
